...
while t1-t0>0.001
    tm=(t1+t0)/2;
    [X0,Y0,F0,qx0,qy0,theta0]=calculate_t(t0);
    [X1,Y1,F1,qx1,qy1,theta1]=calculate_t(t1);
    [Xm,Ym,Fm,qxm,qym,thetam]=calculate_t(tm);
...
%% 计算t
function [X,Y,F,qx,qy,theta]=calculate_t(t) 
        ...
        qx=(X(1:222,:)+X(2:223,:))/2;
        qy=(Y(1:222,:)+Y(2:223,:))/2;
        theta=-atan((Y(2:223,:)-Y(1:222,:))./(X(2:223,:)-X(1:222,:)));
        dist=[];
        for i=1:222
